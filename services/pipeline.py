...
from bot.keyboards import queue_review_keyboard
...
from services.review_queue import pending_projects, queue_position
from services.social_card import generate_social_card
from services.telegram_cleanup import remember_message
...
async def process_raw_signal(
    ...
    async with get_session() as session:
        if await is_archived(session, dedup_hash, workspace_id):
        ...
        queue_items = await pending_projects(session)
        position = queue_position(queue_items, project.id)
        # Only the first pending item is shown. Remaining candidates stay in
        # the database and are opened with queue navigation buttons.
        if position and position[0] == 1:
            request = ReviewRequest(project_id=project.id)
            session.add(request)
            if draft.image_path:
                try:
                    ...
                    remember_message(request, photo_message.message_id)
                    # GitHub Actions has an ephemeral filesystem. Telegram file IDs
                    # are durable for this bot and can be sent again by the Vercel
                    # webhook when the draft is approved later.
                    if social_card:
                        draft.image_path = photo_message.photo[-1].file_id
            ...
            message = await bot.send_message(
                ...
                reply_markup=queue_review_keyboard(project.id, *position),
            )
