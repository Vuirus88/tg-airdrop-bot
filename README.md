...
saving provider credentials in the website settings. The current dashboard is
owner-only and intentionally accepts localhost requests only. Do not expose it
to the internet before user authentication and tenant isolation are added.

The database already contains the SaaS foundation: users, workspaces,
workspace-scoped projects/settings, and separate encrypted Telegram/X
integration records. The current local dashboard uses the seeded personal
workspace only; authentication and workspace selection are the next required
step before public access.

The optional session API is available at `/api/auth/register`,
`/api/auth/login`, `/api/auth/logout`, and `/api/auth/me`. Keep
`WEB_AUTH_MODE=off` during local development; switch it to `required` only
after connecting the login screen.

The web process can start without Telegram credentials. Telegram is an optional
integration for the future SaaS, while `python -m bot.main` validates the three
personal-bot settings before polling. The target boundaries and free-first
deployment assumptions are documented in `docs/architecture.md`.
...
- Add authentication and workspace selection middleware.
- Add individual subscription plans and per-workspace feature limits.
- Publish through each workspace's Telegram/X integration records.
- Add quotas, audit log, rate limits, and billing state.
- Move to PostgreSQL before opening the dashboard to multiple users.
